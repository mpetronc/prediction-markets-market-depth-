"""Discover exact Kalshi counterparts for Polymarket SOL hourly strikes."""
...
def kalshi_event_ticker(expiration_utc: str) -> str:
    ...
    return f"KXSOLD-{local_expiration.strftime('%y%b%d%H').upper()}"


def kalshi_market_ticker(expiration_utc: str, strike_text: str) -> str:
    strike = Decimal(strike_text)
    # SOL hourly contracts use four-decimal threshold boundaries. Therefore,
    # "$X or above" is encoded as a strict threshold at X - $0.0001.
    threshold = strike - Decimal("0.0001")
    return f"{kalshi_event_ticker(expiration_utc)}-T{threshold:.4f}"
...
def normalize_market(
    ...
    reasons: list[str] = []
    if event.get("series_ticker") != "KXSOLD":
...
def main(run_log: RunLog) -> None:
    ...
    run_log.info(
        "Discovery configured venue=kalshi family=hourly_fixed_strike asset=SOL "
