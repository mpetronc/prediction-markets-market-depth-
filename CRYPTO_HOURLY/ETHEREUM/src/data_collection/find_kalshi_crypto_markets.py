"""Discover exact Kalshi counterparts for Polymarket ETH hourly strikes."""
...
def kalshi_event_ticker(expiration_utc: str) -> str:
    ...
    return f"KXETHD-{local_expiration.strftime('%y%b%d%H').upper()}"
...
def normalize_market(
    ...
    reasons: list[str] = []
    if event.get("series_ticker") != "KXETHD":
...
def main(run_log: RunLog) -> None:
    ...
    run_log.info(
        "Discovery configured venue=kalshi family=hourly_fixed_strike asset=ETH "
