"""Discover the matching Kalshi ETH 15-minute markets from Predexon."""
...
def expected_market_tickers() -> list[str]:
    ...
    while end <= WINDOW_END:
        ...
        result.append(f"KXETH15M-{suffix}-{local_end.strftime('%M')}")
...
def normalize_market(market: dict) -> tuple[dict | None, str]:
    ...
    reasons: list[str] = []
    if event.get("series_ticker") != "KXETH15M":
...
def main(run_log: RunLog) -> None:
    ...
    run_log.info(
        "Discovery configured venue=kalshi family=up_down_15m asset=ETH "
