"""Discover ETH hourly fixed-strike markets from Predexon."""
...
def parse_strike(title: object) -> Decimal | None:
    match = re.search(
        r"ethereum\s+above\s+\$?([0-9][0-9,]*(?:\.[0-9]+)?)",
        str(title),
        flags=re.IGNORECASE,
    )
    if not match:
...
def normalize_market(market: dict) -> tuple[dict | None, str]:
    ...
    if "eth/usdt 1 hour candle" not in description.lower():
        reasons.append("resolution is not the ETH/USDT 1-hour close")
    ...
    row = {
        ...
        "resolution_source": "Binance ETH/USDT 1h close",
...
def main(run_log: RunLog) -> None:
    ...
    run_log.info(
        "Discovery configured venue=polymarket family=hourly_fixed_strike asset=ETH "
